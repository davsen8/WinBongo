...
# Nov 17-2015 : changed from reading serial # to sequence # to get shiptripstn
...
# actually sequence #  (drs nov 17 2015)
    SeqNumber = hdr[17][1].lstrip()
    if len(SeqNumber)<8 :
            SeqNumber ="00000000"
    outfile = SeqNumber+".d"+Date[2][2:4]
...
# just take the right most list element -1 is that element
    ...
    hdr1["SHIP"]=SeqNumber[0:2]
    hdr1["TRIP"]=SeqNumber[2:5]
    hdr1["STN"]=SeqNumber[5:8]
...
def Hdr4_Dict_to_String(hdr4):
    hdr4_out= hdr4["SHIP"].strip().zfill(2)+ hdr4["TRIP"].strip().zfill(3)+ hdr4["STN"].strip().zfill(3)+' '+ hdr4["RECORDS"].strip().zfill(6)+\
...
def Hdr8_Dict_to_String(hdr8):
    ...
    hdr8_out= hdr8["SHIP"].strip().zfill(2)+ hdr8["TRIP"].strip().zfill(3)+ hdr8["STN"].strip().zfill(3)+' '+ '{:<1}'.format(hdr8["CLOUD"])+\
...
#        dialog = wx.FileDialog(None, message="Save As", defaultDir="",defaultFile=filename,style=wx.FD_SAVE |wx.FD_OVERWRITE_PROMPT)
